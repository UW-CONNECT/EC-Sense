...
function [estimated_soil_moisture] = compute_vwc_from_pathlosses(tag_depth,reader_height,PL1,PL2)
    VWC_all = 0.0:0.001:0.4;
    
    PL_offset = 7.6314; %10cm battery mean of D1
    % PL_offset = 6.2125; %5cm 50cm mean of D1
    % PL_offset = 5.5879; %5cm 30cm mean of D1
    % PL_offset = 5.102; %10cm 30cm mean of D1
    % PL_offset = 0;

    PL1 = PL1-PL_offset; PL2 = PL2-PL_offset;
    ...
    tag_loc.x = tag_distance * 0;
    tag_loc.y = tag_distance * 0;
    tag_loc.z = tag_depth;
    ...
    for i = 1:length(VWC_all)
        ...
        reader_loc.z = sqrt((3e8/freq/4/pi)^2*10^(PL1/10) - dis_h^2);
        if dis_h == 0
            inc_d = 0;
        else
            syms x
    ...
    %     fprintf("reader height: %.2f m, effective reader loc: %.2f m\n", reader_loc.z, eff_reader_loc_z);
        ...
        loss_diff = loss_tag_in_soil - loss_top_tag;
    
    %     equ_diff_all(i) = abs(10^(PL2/10) - (sqrt(10^(PL1/10))+4*pi*freq*soil_n*path_len_soil/3e8)^2*exp(2*alpha*path_len_soil)*1/(1 - R));
    
        equ_diff_all(i) = abs(PL2 - 10*log10((sqrt(10^(PL1/10))+4*pi*freq*soil_n*path_len_soil/3e8)^2*exp(2*alpha*path_len_soil)*1/(1 - R)));
    ...
    %%
    % figure(1);clf;
    % plot(VWC_all, n_all, 'linewidth', 2);
    % grid on;
    % xlabel('VWC');
    ...
    % 
    % figure(2);clf;
    % plot(VWC_all, alpha_all, 'linewidth', 2);
    % grid on;
    % xlabel('VWC');
    % ylabel('Attenuation coefficient (\alpha)');
    % set(gca,'FontSize',24);
    % 
    % figure(3);clf;
    % plot(VWC_all, loss_refraction_all, 'linewidth', 2);
    % grid on;
    % xlabel('VWC');
    % ylabel('Refraction loss (L_R)');
    % set(gca,'FontSize',24);
    % 
    % figure(4);clf;
    % plot(VWC_all, equ_diff_all, 'linewidth', 2);
    % grid on;
    % xlabel('VWC');
    % ylabel('Loss function (dB)');
    % set(gca,'FontSize',24);
    
    [val, id] = min(equ_diff_all);
    estimated_soil_moisture = VWC_all(id);
    fprintf("Soil moisture: %.3f, min loss: %.3f\n", estimated_soil_moisture, val);
end
